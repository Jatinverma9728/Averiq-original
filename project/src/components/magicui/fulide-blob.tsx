...
float rayMarch(vec3 rayOrigin, vec3 ray) {
    float t = 0.0;
    for (int i = 0; i < 48; i++) { // reduced from 100 to 48 for performance
...
void main() {
    ...
    vec3 color = vec3(0.0); // background black
...
function LavaLampShader({ color = "white" }) {
  const meshRef = useRef();
  // Add a fallback for useThree in case context is not available
  let size = { width: 1, height: 1 };
  try {
    size = useThree((state) => state.size);
  } catch (e) {
    // fallback for SSR or context issues
  }
  ...
  const uniforms = useMemo(
    () => ({
      ...
      blobColor: { value: new THREE.Color(color) }, // new
    }),
    [color]
  );
  ...
  return (
    <mesh ref={meshRef}>
      <planeGeometry args={[3, 3]} /> {/* reduced from [5, 5] */}
      <shaderMaterial
        uniforms={uniforms}
...
export const LavaLamp = ({ color = "white", opacity = 0.95 }) => {
  ...
  return (
    <div
      style={{
        ...
        opacity,
        zIndex: 1,
        pointerEvents: "none", // optimization: don't process pointer events
      }}
    >
      <Canvas
        camera={{
        ...
        dpr={[1, 1.5]} // limit device pixel ratio for perf
        frameloop="always" // restore animation
      >
        <LavaLampShader color={color} />
      </Canvas>
    </div>
  );
};
